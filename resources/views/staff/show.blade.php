...
<div class="dash-wrap">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h1 class="dash-heading mb-0">Personnel Profile</h1>
                <p class="dash-sub mb-0">Detailed view for {{ $staff->full_name }}</p>
            </div>
        </div>
        <div class="d-flex gap-3">
            <a href="{{ route('staff.edit', $staff->staff_id) }}" class="btn-dash btn-ghost px-4">
        ...
        <div class="col-xl-3">
            <div class="dash-panel border-0 shadow-sm text-center p-4 h-100">
                <div class="profile-photo-wrap mb-4 mx-auto">
                    @if($staff->photo_url)
                        <img src="{{ $staff->photo_url }}" alt="Staff Photo" class="profile-photo shadow-sm">
                    @else
                ...
                <h4 class="profile-name mb-1">{{ $staff->full_name }}</h4>
                ...
                <div class="text-start ps-2">
                    <div class="sidebar-item mb-4">
                        ...
                        <div class="sidebar-value">{{ $staff->employee_number ?: ($staff->employee_id ?: 'N/A') }}</div>
                    </div>
                    <div class="sidebar-item mb-4">
                        <label class="sidebar-label">Department</label>
                        <div class="sidebar-value">{{ $staff->department->name ?? 'Unassigned' }}</div>
                    </div>
                    <div class="sidebar-item mb-0">
                        <label class="sidebar-label">Current Position</label>
                        <div class="sidebar-value">{{ $staff->jobPosition->name ?? ($staff->designation ?: 'Staff Member') }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xl-9">
            <div class="dash-panel border-0 shadow-sm p-0 h-100">
                <div class="panel-nav border-bottom px-4" style="background: #fafbfc;">
                    <div class="d-flex" style="gap: 50px;"> {{-- Massive gap for clarity --}}
                    ...
                    <div class="tab-section" id="personal">
                        <div class="row g-4">
                            <div class="col-md-4">
                                ...
                                <div class="detail-value text-capitalize">{{ $staff->gender ?: '—' }}</div>
                            </div>
                            <div class="col-md-4">
                                <label class="detail-label">Date of Birth</label>
                                <div class="detail-value">{{ $staff->date_of_birth ? $staff->date_of_birth->format('M d, Y') : '—' }}</div>
                            </div>
                            <div class="col-md-4">
                                <label class="detail-label">Marital Status</label>
                                <div class="detail-value text-capitalize">{{ $staff->marital_status ?: '—' }}</div>
                            </div>
                            <div class="col-md-4">
                                <label class="detail-label">National ID</label>
                                <div class="detail-value">{{ $staff->national_id_number ?: '—' }}</div>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="detail-value">{{ $staff->religion ?: '—' }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tab-section d-none" id="professional">
                        <div class="row g-4">
                            <div class="col-md-6">
                                ...
                                <div class="detail-value">{{ $staff->date_of_joining ? $staff->date_of_joining->format('M d, Y') : ($staff->joining_date ?: '—') }}</div>
                            </div>
                            <div class="col-md-6">
                                <label class="detail-label">Employment Type</label>
                                <div class="detail-value text-capitalize">{{ str_replace('_', ' ', $staff->employment_type ?: 'Full Time') }}</div>
                            </div>
                            <div class="col-md-12">
                            ...
                            <div class="col-md-12">
                                <label class="detail-label">Experience & Skills</label>
                                <div class="detail-value">{{ $staff->experience ?: 'No experience data available.' }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tab-section d-none" id="contact">
                        <div class="row g-4">
                            <div class="col-md-6">
                                ...
                                <div class="detail-value text-lowercase">{{ $staff->work_email ?: ($staff->email ?: '—') }}</div>
                            </div>
                            <div class="col-md-6">
                                <label class="detail-label">Primary Phone</label>
                                <div class="detail-value">{{ $staff->phone_primary ?: ($staff->phone ?: '—') }}</div>
                            </div>
                            <div class="col-md-12">
                                <label class="detail-label">Current Residence</label>
                                <div class="detail-value">{{ $staff->current_address ?: ($staff->address ?: '—') }}</div>
                            </div>
                            <div class="col-md-4">
                                <label class="detail-label">City / Town</label>
                                <div class="detail-value">{{ $staff->city ?: '—' }}</div>
                            </div>
                            <div class="col-md-4">
                                <label class="detail-label">County / State</label>
                                <div class="detail-value">{{ $staff->county ?: '—' }}</div>
                            </div>
                            <div class="col-md-4">
                                <label class="detail-label">Country</label>
                                <div class="detail-value">{{ $staff->country ?: '—' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
...
.status-indicator { position: absolute; bottom: 5px; right: 5px; width: 22px; height: 22px; border: 4px solid #fff; border-radius: 50%; }
