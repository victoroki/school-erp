...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="text-dark font-weight-bold"><i class="fas fa-edit text-success mr-2"></i>Edit Income</h1>
                </div>
    ...
    <div class="content px-3">
        ...
        <div class="card border-0 shadow-sm rounded-lg overflow-hidden">
            ...
            {!! Form::model($income, ['route' => ['income.update', $income->income_id], 'method' => 'PUT', 'files' => true]) !!}
            <div class="card-body">
                <div class="row">
                    <!-- Income Date Field -->
                    <div class="form-group col-sm-6">
                        <label class="font-weight-bold">Income Date <span class="text-danger">*</span></label>
                        {!! Form::date('income_date', $income->income_date ? $income->income_date->format('Y-m-d') : \Carbon\Carbon::now()->format('Y-m-d'), ['class' => 'form-control border-0 bg-light rounded-lg', 'required']) !!}
                    </div>
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        ], null, ['class' => 'form-control border-0 bg-light rounded-lg', 'id' => 'payment_method']) !!}
                    </div>
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        @if($income->attachment)
                            <small class="form-text text-muted">
                                Current file: <a href="{{ asset('storage/' . $income->attachment) }}" target="_blank">{{ basename($income->attachment) }}</a>
                            </small>
                        @endif
                    </div>
                </div>
            ...
            <div class="card-footer bg-light py-4 text-center">
                {!! Form::submit('Update Income Entry', ['class' => 'btn btn-success rounded-pill px-5 py-2 font-weight-bold shadow-sm']) !!}
                <a href="{{ route('income.show', $income->income_id) }}" class="btn btn-outline-secondary rounded-pill px-5 py-2 ml-2">Cancel</a>
            </div>
            {!! Form::close() !!}
        </div>
    </div>
