    ...
    <div class="row align-items-center mb-4">
        <div class="col-md-7">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h1 class="dash-heading">Cashflow Statement</h1>
        ...
        <div class="col-md-5 text-md-end mt-3 mt-md-0">
            <form action="{{ route('financial-reports.cashflow') }}" method="GET" class="d-flex justify-content-md-end gap-2 flex-wrap">
            ...
            <div class="mt-2">
                <a href="{{ route('financial-reports.cashflow-pdf', ['start_date' => $startDate, 'end_date' => $endDate]) }}"
        ...
        <div class="col-md-4 mb-3 mb-md-0">
            <div class="stat-card">
                ...
                <div class="stat-info w-100">
                    ...
                    <span class="stat-value text-emerald">KES {{ number_format($totalInflows, 2) }}</span>
                </div>
            </div>
        </div>
        <div class="col-md-4 mb-3 mb-md-0">
            <div class="stat-card">
                ...
                <div class="stat-info w-100">
                    ...
                    <span class="stat-value text-rose">KES {{ number_format($totalOutflows, 2) }}</span>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="stat-card {{ $netCashflow >= 0 ? 'border-emerald-light' : 'border-rose-light' }}">
                <div class="stat-icon {{ $netCashflow >= 0 ? 'bg-indigo-light text-indigo' : 'bg-rose-light text-rose' }}"><i class="fas fa-balance-scale"></i></div>
                <div class="stat-info w-100">
                    <span class="stat-label">Net Cash Flow</span>
                    <span class="stat-value {{ $netCashflow >= 0 ? 'text-indigo' : 'text-rose' }}">KES {{ number_format($netCashflow, 2) }}</span>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="dash-panel h-100">
                <div class="dash-panel-header bg-emerald-light border-0">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <h3 class="dash-panel-title text-emerald">Cash Inflows</h3>
                    </div>
                </div>
                <div class="dash-panel-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            <thead>
                                <tr>
                                    <th class="pl-4">Category / Item</th>
                                    <th class="text-right pr-4">Amount</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="bg-slate-light">
                                    <td class="pl-4 font-weight-bold" colspan="2"><i class="fas fa-graduation-cap me-2 text-slate"></i> Fee Collections</td>
                                </tr>
                                <tr>
                                    <td class="pl-4 pl-md-5">Total Student Fees</td>
                                    <td class="text-right pr-4 font-weight-bold text-dark">KES {{ number_format($totalFees, 2) }}</td>
                                </tr>
                                <tr class="bg-slate-light">
                                    <td class="pl-4 font-weight-bold" colspan="2"><i class="fas fa-briefcase me-2 text-slate"></i> Other Income</td>
                                </tr>
                                @forelse($income->groupBy('category_id') as $categoryId => $items)
                                    <tr>
                                        <td class="pl-4 pl-md-5">{{ $items->first()->category ? $items->first()->category->name : 'Uncategorized' }}</td>
                                        <td class="text-right pr-4 font-weight-bold text-dark">KES {{ number_format($items->sum('amount'), 2) }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="2" class="text-center py-3 text-muted">No other income recorded.</td>
                                    </tr>
                                @endforelse
                                
                                <tr class="border-top-thick">
                                    <td class="pl-4 font-weight-bold" style="font-size: 1.1rem;">Total Inflows</td>
                                    <td class="text-right pr-4 font-weight-bold text-emerald" style="font-size: 1.1rem;">KES {{ number_format($totalInflows, 2) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="dash-panel h-100">
                <div class="dash-panel-header bg-rose-light border-0">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <h3 class="dash-panel-title text-rose">Cash Outflows</h3>
                    </div>
                </div>
                <div class="dash-panel-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                @forelse($expenses->groupBy('category_id') as $categoryId => $items)
                                    <tr>
                                        <td class="pl-4 font-weight-bold">
                                            <i class="fas fa-tags me-2 text-slate opacity-50"></i>
                                            {{ $items->first()->category ? $items->first()->category->name : 'Uncategorized' }}
                                        </td>
                                        <td class="text-right pr-4 font-weight-bold text-dark">KES {{ number_format($items->sum('amount'), 2) }}</td>
                                    </tr>
                                    @foreach($items as $expense)
                                    <tr class="bg-light">
                                        <td class="pl-4 pl-md-5 text-muted small py-2">↳ {{ $expense->description ?: 'No description' }} ({{ $expense->expense_date->format('d M') }})</td>
                                        <td class="text-right pr-4 text-muted small py-2">KES {{ number_format($expense->amount, 2) }}</td>
                                    </tr>
                                    @endforeach
                                @empty
                                    <tr>
                                ...
                                <tr class="border-top-thick mt-auto">
                                    <td class="pl-4 font-weight-bold" style="font-size: 1.1rem;">Total Outflows</td>
                                    <td class="text-right pr-4 font-weight-bold text-rose" style="font-size: 1.1rem;">KES {{ number_format($totalOutflows, 2) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
.border-top-thick { border-top: 2px solid var(--border) !important; background: #f8fafc; }
